...
try:
    ...
    AUTH_OK = False
    print(f"[WARN] auth.py yüklenemedi: {_e}")
...
for key, default in [
    ...
    ("pending_action", None), ("admin_msg", None),
    ("admin_tab", "onay"),
]:
    if key not in st.session_state:
        st.session_state[key] = default

# Auth yoksa direkt giriş yap
...
def section_header(title, subtitle="", g1=None, g2=None):
    ...
    <div style='margin-bottom:28px;'>
        <div style='font-family:Syne;font-size:1.85rem;font-weight:800;margin:0;
            ...
            <span style='color:{c1};'>{first} </span><span style='color:{c2};'>{rest}</span>
        </div>
        {f"<div style='font-family:DM Mono;font-size:.68rem;color:{T['text_muted']};letter-spacing:.12em;margin-top:8px;text-transform:uppercase;'>{subtitle}</div>" if subtitle else ""}
    </div>"""

def hr():
    return "<div style='height:1px;background:linear-gradient(90deg,transparent,rgba(201,168,76,.15),transparent);margin:20px 0;'></div>"

def _hmetric(label, value, color=None):
    c = color or GOLD
    return f"""<div style='background:linear-gradient(135deg,{T["bg_card"]},{T["bg_card2"]});
        border:1px solid {T["border"]};border-radius:14px;padding:18px 20px;'>
        <div style='font-family:DM Mono;font-size:.58rem;color:{T["text_muted"]};
            text-transform:uppercase;letter-spacing:.18em;margin-bottom:8px;'>{label}</div>
        <div style='font-family:Syne;font-size:1.8rem;font-weight:800;color:{c};'>{value}</div>
    </div>"""

def plotly_layout(**kw):
    """FIX: PL() yerine plotly_layout() — isim çakışması engellendi"""
    b = dict(
...
def show_auth_page():
    if AUTH_OK and not admin_exists():
        ...
        <div style='max-width:520px;margin:60px auto 0 auto;
            ...
            <div style='font-family:Syne;font-size:1.1rem;font-weight:800;
                        color:{ORANGE};margin-bottom:10px;'>⚙️ Kurulum Gerekli</div>
            <div style='font-family:DM Mono;font-size:.72rem;color:{T['text_secondary']};line-height:2;'>
                Henüz yönetici hesabı oluşturulmadı.<br>
                Terminalde çalıştırın:
            </div>
            <div style='background:rgba(0,0,0,.3);border-radius:8px;padding:12px 16px;margin-top:12px;
                        font-family:DM Mono;font-size:.8rem;color:{CYAN};letter-spacing:.05em;'>
                python setup.py
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col:
        ...
        <div style='text-align:center;padding:36px 0 24px 0;animation:fadeIn .5s ease;'>
            ...
            <div style='font-family:DM Mono;font-size:.58rem;color:{T['text_muted']};
                        letter-spacing:.22em;text-transform:uppercase;margin-top:4px;'>Analiz Sistemi</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if st.session_state.auth_tab == "login":
            ...
            with st.form("register_form", clear_on_submit=True):
                ...
                r_role = st.selectbox("🎭 Rol", ["viewer","analyst"],
                    format_func=lambda x: {"viewer":"👁️ Viewer","analyst":"📊 Analist"}[x])
                ra, rb = st.columns(2)
                ...
                if r_submit:
                    err = None
                    if not all([r_display, r_username, r_email, r_pw1, r_pw2]): err = "Tüm alanları doldurun."
                    elif " " in r_username: err = "Username boşluk içeremez."
                    elif r_pw1 != r_pw2: err = "Şifreler eşleşmiyor."
                    if err:
                        st.error(err)
                    elif AUTH_OK:
                        result = register_user(r_username, r_email, r_pw1, r_display, r_role)
                        if result["success"]: st.success("✅ Kayıt başarılı! Admin onayı bekleniyor.")
...
def adapt_real_data(df):
    """FIX: robust adapt — eksik kolon varsa fallback"""
    df = df.copy()
    np.random.seed(42)
    n = len(df)
    sehir = ['Istanbul','Ankara','Izmir','Bursa','Antalya',
...
def filter_data(risk_filtre_tuple, segment_filtre_tuple, risk_min, risk_max):
    """FIX: parametre olarak tuple alır (hashable for cache)"""
    df_all, _ = load_main_data()
    risk_f = list(risk_filtre_tuple)
    seg_f  = list(segment_filtre_tuple)
    # FIX: risk_seviyesi string karşılaştırması
    mask = (
...
with st.sidebar:
    ...
    # FIX: ASCII-safe filtre seçenekleri
    ...
    # Fallback: filtre sonucu boşsa tüm veriyi kullan
    if len(df) == 0:
        df = df_main.copy()
    ...
    <div style='font-family:DM Mono;font-size:.58rem;color:{T['text_muted']};
        ...
        <span style='color:{GOLD};font-weight:600;'>{len(df):,}</span> müşteri seçildi
    </div>
    """, unsafe_allow_html=True)
...
if sayfa == "📊 Overview":
    ...
    with w1:
        ...
        _pl = plotly_layout(); _pl['margin'] = dict(l=30, r=20, t=45, b=30)
    ...
    with w2:
        ...
        _pl2 = plotly_layout(); _pl2['margin'] = dict(l=10, r=10, t=45, b=10)
    ...
    st.markdown(section_header("Monthly Transaction Volume", "Time series — transaction volume trend", None, GOLD), unsafe_allow_html=True)
    ...
    st.markdown(section_header("Segment Distribution", "Customer segment breakdown", None, CYAN), unsafe_allow_html=True)
    ...
    st.markdown(section_header("Category Averages", "Average spending by category", None, ORANGE), unsafe_allow_html=True)
    ...
    st.markdown(section_header("Spending × Risk", "Fraud detection scatter analysis", None, RED), unsafe_allow_html=True)
    fig = go.Figure()
    for ft, col_c in [('Normal', GREEN), ('Suheli', RED)]:
        mask = df['fraud_tahmini'].astype(str).str.contains(ft.replace('Suheli', 'pheli') if ft == 'Suheli' else ft)
        if mask.sum() > 0:
    ...
    st.markdown(section_header("Trend Analysis", "Time series and category", None, CYAN), unsafe_allow_html=True)
    ...
    with tab1:
        ...
        if api_aylik:
            ...
            donemler = pd.date_range(end=pd.Timestamp.today(), periods=24, freq='ME')
            base  = float(df['toplam_harcama'].sum()) / 24 if len(df) > 0 else 500000
            da = pd.DataFrame({'donem': donemler.strftime('%Y-%m'),
    ...
    st.markdown(section_header("Customer Analysis", "Segment, demographics and behavior", None, GREEN), unsafe_allow_html=True)
...
# ── RİSK & FRAUD ──
    ...
    st.markdown(section_header("Risk & Fraud", "Real-time ML predictions", None, RED), unsafe_allow_html=True)

    ml_df   = load_ml_data()
    ml_ozet = load_ml_ozet()

    # FIX: ML yoksa demo data ile çalış, st.stop() kaldırıldı
    if ml_df.empty:
        st.info("ℹ️ ML verisi bulunamadı — demo data gösteriliyor. `python src/ml_model.py` çalıştırın.")
        ml_df = df_main.copy()
        if 'fraud_skoru' not in ml_df.columns:
            ml_df['fraud_skoru'] = ml_df['risk_skoru'] * 1.2
        if 'fraud_tahmini' not in ml_df.columns:
            ml_df['fraud_tahmini'] = np.where(ml_df['risk_skoru'] > 30, '🔴 Yüksek Risk', 'Normal')
        if 'churn_skoru' not in ml_df.columns:
            ml_df['churn_skoru'] = np.random.exponential(15, len(ml_df)).clip(0, 100)
    ...
    st.markdown("<div style='height:24px;'></div>", unsafe_allow_html=True)
    tab1, tab2, tab3, tab4 = st.tabs(["📊 Distribution", "🚨 High Risk List", "📉 Churn", "🔬 Detail Analysis"])
        ...
        with c1:
            if 'fraud_skoru' in ml_df.columns:
                ...
                if 'fraud_tahmini' in ml_df.columns:
                    for lvl, col_c in [('Normal', GREEN), ('pheli', ORANGE), ('ksek', RED)]:
                        sub = ml_df[ml_df['fraud_tahmini'].astype(str).str.contains(lvl)]['fraud_skoru']
                        label = {'Normal': 'Normal', 'pheli': '⚠️ Şüpheli', 'ksek': '🔴 Yüksek Risk'}[lvl]
                        if len(sub) > 0:
                            fig.add_trace(go.Histogram(x=sub, name=label, marker_color=col_c, opacity=.75, nbinsx=30))
                else:
                    fig.add_trace(go.Histogram(x=ml_df['fraud_skoru'], marker_color=GOLD, opacity=.8, nbinsx=30))
                fig.update_layout(title="📊 Fraud Score Distribution (ML)", barmode='overlay', height=380, **plotly_layout())
                st.plotly_chart(fig, use_container_width=True)
        with c2:
            ...
            fig2.add_annotation(text=f"<b>{fraud_pct}%</b>", x=.5, y=.5, showarrow=False,
    ...
    with tab2:
        if 'fraud_skoru' in ml_df.columns:
            ...
            fig.update_layout(title="🚨 Top 20 High Risk Customers (ML Score)", height=340, **plotly_layout())
            st.plotly_chart(fig, use_container_width=True)
    ...
    with tab3:
        if 'churn_skoru' in ml_df.columns:
            ...
            fig.update_layout(title="📉 Highest Churn Risk Customers", height=340, **plotly_layout())
            st.plotly_chart(fig, use_container_width=True)
            c1, c2 = st.columns(2)
            with c2:
                fig3 = go.Figure(go.Histogram(x=ml_df['churn_skoru'], nbinsx=30,
                    marker_color=PURPLE, opacity=.8))
                fig3.update_layout(title="📊 Churn Score Distribution", height=300, **plotly_layout())
                st.plotly_chart(fig3, use_container_width=True)
        else:
            st.info("Churn skoru bulunamadı.")
        ...
        with c2:
            if 'tx_hata_oran' in ml_df.columns and 'fraud_skoru' in ml_df.columns:
                ...
                fig2 = px.scatter(smp, x='tx_hata_oran', y='fraud_skoru',
                    color=color_col,
                    title="⚠️ Error Rate × Fraud Score")
                fig2.update_layout(height=380, **plotly_layout())
                st.plotly_chart(fig2, use_container_width=True)


elif sayfa == "🗺️ Geographic Analysis":
    st.markdown(section_header("Geographic Analysis", "Regional distribution", None, PURPLE), unsafe_allow_html=True)
    ...
    with tab2:
        ...
        fig.update_layout(title="🗺️ Total Volume", paper_bgcolor='rgba(0,0,0,0)',
...
# ── AI İÇGÖRÜLERİ ──
    ...
    st.markdown(section_header("AI Insights", "Model performance and detections", None, CYAN), unsafe_allow_html=True)

    ml_df   = load_ml_data()
    metrics = load_model_metrics()

    tab1, tab2, tab3 = st.tabs(["📊 Model Performance", "🚨 Suspicious Detections", "🔍 Feature Analysis"])
            ...
            with c2:
                ...
                if cm and len(cm) == 2:
                    ...
                    fig2 = go.Figure(go.Heatmap(
                        z=z_vals, x=['Pred: Normal', 'Pred: Fraud'],
    ...
    with tab2:
        if not ml_df.empty and 'fraud_skoru' in ml_df.columns:
            ...
            for i, (_, row) in enumerate(yr.iterrows()):
                ...
                sev = "🔴 Kritik" if fs > 60 else "🟡 Şüpheli"
                signals = []
                if float(row.get('tx_gece_oran', 0)) > 0.3:  signals.append("🌙 Gece işlem fazla")
                if float(row.get('tx_hata_oran', 0)) > 0.1:  signals.append("⚠️ Yüksek hata oranı")
                if float(row.get('dark_web_oran', 0)) > 0:   signals.append("🕶️ Dark web kartı")
                ...
                <div style='background:linear-gradient(135deg,rgba(255,69,96,.05),{T["bg_card"]});
                            ...
                            padding:14px 18px;margin:8px 0;border:1px solid rgba(255,69,96,.12);'>
                    <div style='display:flex;justify-content:space-between;margin-bottom:8px;'>
                        <span style='font-family:Syne;font-weight:700;color:{T["text_primary"]};font-size:.95rem;'>
                            Müşteri #{int(row.get("client_id", i))}
                        </span>
                        <span style='font-family:DM Mono;font-size:.65rem;color:{bc};
                                     background:{bc}18;padding:2px 10px;border-radius:20px;
                    ...
                    <div style='font-family:DM Mono;font-size:.62rem;color:{T["text_muted"]};line-height:1.7;'>
                        {sig_html}
                    </div>
                    <div style='width:100%;background:{T["border"]};height:3px;border-radius:3px;margin-top:10px;'>
                        <div style='width:{min(fs,100):.0f}%;background:linear-gradient(90deg,{ORANGE},{bc});height:100%;border-radius:3px;'></div>
                    </div>
                </div>"""
                with (ca if i % 2 == 0 else cb):
                    st.markdown(card, unsafe_allow_html=True)
        else:
            st.info("ML verisi bulunamadı.")

    with tab3:
        if metrics and 'feature_importance' in metrics:
            fi = pd.DataFrame(metrics['feature_importance'])
            fig = go.Figure(go.Bar(
                y=fi['feature'].head(15), x=fi['importance'].head(15), orientation='h',
                marker=dict(color=fi['importance'].head(15),
                            colorscale=[[0, 'rgba(201,168,76,.3)'], [1, GOLD]]),
                text=[f"{v:.3f}" for v in fi['importance'].head(15)], textposition='outside',
...
# ── MÜŞTERİ DETAY ──
    ...
    st.markdown(section_header("Customer Detail", "360° customer profile and ML predictions"), unsafe_allow_html=True)

    @st.cache_data(ttl=600, show_spinner=False)
    def load_detail():
        """FIX: Her tablo ayrı try/except — biri crash etse diğerleri çalışır"""
        ml_df2   = pd.DataFrame()
        ...
        # CSV dosyaları
        for csv_path, target, name in [
            (os.path.join(DATA_DIR, "users_data.csv"),  "users",  "users_df"),
            (os.path.join(DATA_DIR, "cards_data.csv"),  "cards",  "cards_df"),
        ]:
            try:
                if os.path.exists(csv_path):
                    ...
                    if name == "users":
                        for c in ['per_capita_income', 'yearly_income', 'total_debt']:
                            if c in tmp.columns:
                                tmp[c] = tmp[c].astype(str).str.replace('$', '', regex=False).str.replace(',', '', regex=False)
                                tmp[c] = pd.to_numeric(tmp[c], errors='coerce').fillna(0)
                        users_df = tmp
                    else:
                        if 'credit_limit' in tmp.columns:
                            tmp['credit_limit'] = tmp['credit_limit'].astype(str).str.replace('$', '', regex=False).str.replace(',', '', regex=False)
                            tmp['credit_limit'] = pd.to_numeric(tmp['credit_limit'], errors='coerce').fillna(0)
                        cards_df = tmp
            except Exception:
                pass
    ...
    # FIX: Müşteri listesi — her kaynaktan dene
    id_list = []
    if len(ml_df2) > 0:
        id_list = sorted(ml_df2['client_id'].astype(int).tolist())
    elif len(risk_df) > 0:
        id_list = sorted(risk_df['client_id'].astype(int).tolist())
    elif len(df_main) > 0:
        id_list = sorted(df_main['client_id'].astype(int).tolist())
    ...
    with k3: st.metric("⚠️ Risk Skoru",   f"{float(rr['risk_skoru']):.1f}"     if rr  is not None else "—")
    with k4: st.metric("🤖 Fraud Skoru",  f"{float(ml_r.get('fraud_skoru',0)):.1f}"  if ml_r is not None else "—")
    with k5: st.metric("📉 Churn Skoru",  f"{float(ml_r.get('churn_skoru',0)):.1f}"  if ml_r is not None else "—")
    with k6: st.metric("🏦 Kredi Skoru",  f"{int(ur['credit_score'])}"         if ur  is not None else "—")
    ...
    if ml_r is not None:
        ...
        st.markdown(f"""
        <div style='background:linear-gradient(135deg,{T["bg_card"]},{T["bg_card2"]});
                    border:1px solid {T["border"]};border-radius:16px;padding:24px 28px;margin-bottom:20px;'>
            <div style='font-family:DM Mono;font-size:.6rem;color:{T["text_muted"]};
                ...
                🤖 ML Tahmin Özeti — Müşteri #{secili}
            </div>
            <div style='display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:20px;'>
                <div>
                    <div style='font-family:DM Mono;font-size:.58rem;color:{T["text_muted"]};margin-bottom:6px;'>FRAUD TAHMİN</div>
                ...
                <div>
                    <div style='font-family:DM Mono;font-size:.58rem;color:{T["text_muted"]};margin-bottom:6px;'>CHURN TAHMİN</div>
                ...
                <div>
                    <div style='font-family:DM Mono;font-size:.58rem;color:{T["text_muted"]};margin-bottom:6px;'>ANOMALİ</div>
                    <div style='font-family:Syne;font-size:1rem;font-weight:800;color:{"#FF4560" if ml_r.get("iso_tahmin",0)==1 else "#00E396"};'>
...
# ── ADMİN PANELİ ──
    ...
    if not AUTH_OK:
        st.warning("⚠️ Auth modülü yüklenemedi.")
        st.stop()

    st.markdown(section_header("Admin Panel", "User management and system control"), unsafe_allow_html=True)

    # FIX: pending_action işle + rerun
    if st.session_state.get("pending_action"):
        ...
        try:
            if action["type"] == "approve":
                ...
                st.session_state.admin_msg = ("warning", f"⚠️ {action['username']} reddedildi.")
            elif action["type"] == "role_update":
                update_user_role(action["id"], action["role"])
                st.session_state.admin_msg = ("success", f"✅ {action['username']} rol → {action['role']}")
            elif action["type"] == "delete":
                delete_user(action["id"])
                st.session_state.admin_msg = ("warning", f"🗑️ {action['username']} silindi.")
    ...
    if st.session_state.get("admin_msg"):
        ...
        st.markdown(f"""<div style='background:{_bg};border:1px solid {_br};border-left:4px solid {_cl};
            border-radius:10px;padding:12px 18px;margin-bottom:16px;
            font-family:DM Mono;font-size:.82rem;color:{_cl};'>{_mx}</div>""", unsafe_allow_html=True)
    ...
    _mc5.markdown(_hmetric("🔑 Toplam Giriş",   f"{login_stats['toplam']:,}"), unsafe_allow_html=True)
    ...
    st.markdown(f"<div style='height:1px;background:{T['border']};margin:8px 0 20px 0;'></div>", unsafe_allow_html=True)
    ...
    if _active_tab == "onay":
            ...
            for _u in pending:
                ...
                with _pc1:
                    st.markdown(f"""<div style='padding:8px 0;'>
                        <div style='font-family:Syne;font-size:.88rem;font-weight:700;color:{T["text_primary"]};'>{_u["username"]}</div>
                        <div style='font-family:DM Mono;font-size:.6rem;color:{T["text_muted"]};'>{_u["email"]}</div>
                ...
                with _pc3:
                    _nr = st.selectbox("", ["viewer", "analyst", "admin"],
                        index=["viewer", "analyst", "admin"].index(_u["role"]) if _u["role"] in ["viewer", "analyst", "admin"] else 0,
                        key=f"rp_{_u['id']}", label_visibility="collapsed")
                with _pc4:
                    st.markdown('<div class="btn-green">', unsafe_allow_html=True)
                    if st.button("✅", key=f"ap_{_u['id']}", use_container_width=True):
                        st.session_state.pending_action = {"type": "approve", "id": _u["id"], "role": _nr, "username": _u["username"]}
                        st.rerun()
                    st.markdown("</div>", unsafe_allow_html=True)
                with _pc5:
                    st.markdown('<div class="btn-red">', unsafe_allow_html=True)
                    if st.button("❌", key=f"rj_{_u['id']}", use_container_width=True):
                        st.session_state.pending_action = {"type": "reject", "id": _u["id"], "username": _u["username"]}
                        st.rerun()
                    st.markdown("</div>", unsafe_allow_html=True)
                st.markdown(f"<div style='height:1px;background:{T['border']};margin:4px 0;'></div>", unsafe_allow_html=True)
        else:
            st.markdown(f"""<div style='background:rgba(0,227,150,.05);border:1px solid rgba(0,227,150,.18);
                border-radius:12px;padding:30px;text-align:center;'>
                <div style='font-size:2rem;margin-bottom:8px;'>✅</div>
                <div style='font-family:Syne;font-size:.9rem;font-weight:700;color:{GREEN};'>Onay bekleyen kullanıcı yok</div>
            </div>""", unsafe_allow_html=True)
        ...
        for _u in all_users:
            ...
            with _uc1:
                st.markdown(f"<div style='font-size:1.2rem;padding-top:8px;text-align:center;'>{_av}</div>", unsafe_allow_html=True)
            with _uc2:
                st.markdown(f"""<div style='padding:5px 0;'>
                    <div style='font-family:Syne;font-size:.82rem;font-weight:700;color:{T["text_primary"]};'>{_u["username"]}</div>
                    <div style='font-family:DM Mono;font-size:.58rem;color:{T["text_muted"]};'>{_u["email"]}</div>
                </div>""", unsafe_allow_html=True)
            with _uc3:
                st.markdown(f"<div style='font-family:DM Mono;font-size:.62rem;color:{T['text_muted']};padding-top:9px;'>{_u.get('display_name') or '—'}</div>", unsafe_allow_html=True)
            with _uc4:
            ...
            with _uc5:
                if _nr2 != _u["role"]:
                    ...
                    if st.button("💾", key=f"sr_{_u['id']}", use_container_width=True):
                        st.session_state.pending_action = {"type": "role_update", "id": _u["id"], "role": _nr2, "username": _u["username"]}
                        st.rerun()
                    st.markdown("</div>", unsafe_allow_html=True)
            with _uc6:
                st.markdown(f"<div style='font-family:DM Mono;font-size:.62rem;padding-top:10px;'><span style='color:{_sc};'>{_sl}</span></div>", unsafe_allow_html=True)
            with _uc7:
                if _u["username"] != st.session_state.username:
                    st.markdown('<div class="btn-red">', unsafe_allow_html=True)
                    if st.button("🗑️", key=f"dl_{_u['id']}", use_container_width=True):
                        st.session_state.pending_action = {"type": "delete", "id": _u["id"], "username": _u["username"]}
                        st.rerun()
                    st.markdown("</div>", unsafe_allow_html=True)
        ...
        with _pw1: _old_pw  = st.text_input("Mevcut Şifre",    type="password", key="old_pw")
        with _pw2: _new_pw  = st.text_input("Yeni Şifre",      type="password", key="new_pw")
        with _pw3: _new_pw2 = st.text_input("Yeni Şifre Tekrar", type="password", key="new_pw2")
        ...
        _ls1.markdown(_hmetric("🔑 Toplam Giriş",  f"{login_stats['toplam']:,}"),         unsafe_allow_html=True)
        _ls2.markdown(_hmetric("✅ Başarılı",       f"{login_stats['basarili']:,}",  GREEN), unsafe_allow_html=True)
        _ls3.markdown(_hmetric("❌ Başarısız",      f"{login_stats['basarisiz']:,}", RED),   unsafe_allow_html=True)
        ...
        if ml_ozet_s:
            ...
            <div style='background:rgba(0,212,255,.05);border:1px solid rgba(0,212,255,.15);
                ...
                <div style='font-family:DM Mono;font-size:.6rem;color:{T["text_muted"]};
                            text-transform:uppercase;margin-bottom:10px;'>🤖 Son ML Çalışması</div>
                <div style='font-family:DM Mono;font-size:.72rem;color:{T["text_secondary"]};line-height:2;'>
                    ...
                    Yüksek Risk: <span style='color:{RED};'>{ml_ozet_s.get("yuksek_risk",0):,}</span>
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with _c1:
            _ep = st.selectbox("API Endpoint", ["/health", "/stats", "/stats/fraud", "/model/metrics"], key="api_ep_sel")
            if st.button("▶️ Test", use_container_width=True, key="api_test_btn"):
                _res = api_get(_ep)
                if _res: st.json(_res if isinstance(_res, dict) else _res[:2])
                else: st.error("❌ Bağlantı başarısız.")
        with _c2:
            st.download_button("⬇️ Tam Müşteri Verisi",
